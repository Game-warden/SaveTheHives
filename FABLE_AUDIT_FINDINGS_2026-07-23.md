...
> **Protocol addendum for the next Fable audit session (added 2026-07-23, Sonnet implementation session):** During live verification of the 2b fix below, we caught a bug this audit's methodology couldn't have found: `submit_checkin()` declared `p_hive_id uuid` in `v2_6_sync.sql`, but the live `hives.id` column is actually `bigint` — a mismatch that meant **every** check-in RPC call had been failing since the v2.6 rollout, silently, because nobody had run one end-to-end. This audit's method (line 6, above) reads SQL migration files and client code, but a migration file describes intent at the time it was written — it can drift from the live schema (later dashboard edits, partial runs, manual fixes) without the repo ever reflecting it. Static review of `v2_6_sync.sql` alone, however careful, would reproduce this exact miss again.
>
> **Add to the testing protocol:** for any finding that touches a Postgres RPC/function signature, don't stop at reading the `.sql` file in the repo — cross-check the parameter types against the *live* column types (e.g. `select pg_typeof(col) from table limit 1`, or `\d table` if a psql-like interface is available), and where dashboard access allows it, actually **invoke the RPC once end-to-end** (not just read that it exists) rather than trusting that the checked-in migration matches production. This applies most to any function marked `SECURITY DEFINER`/`SECURITY INVOKER` or anything gating a core user-facing action (here: Validate, the app's primary CTA) — those are exactly the paths where a silent, untested failure does the most damage before anyone notices.
...
> **New finding, not in Fable's original audit (caught during live verification of the 2b fix, 2026-07-23):** `submit_checkin()`'s `p_hive_id` parameter was declared `uuid` in `v2_6_sync.sql` since that migration first ran, but `hives.id` is actually `bigint` (confirmed live: `select id, pg_typeof(id) from public.hives` → `bigint`). A bigint like `885` can never parse as a UUID, so **every** call to this RPC — i.e. every attempted check-in, by any user, since the v2.6 rollout — would have failed with `invalid input syntax for type uuid`. This wasn't introduced by today's `SECURITY DEFINER` change; it was a dormant, pre-existing bug in the original migration that nobody had caught because the check-in flow had apparently never been tested end-to-end with a real submit. Fixed as part of the same SQL Editor session: dropped the mistyped `submit_checkin(uuid, text, text)` overload and recreated it as `submit_checkin(bigint, text, text)`. Verified live: checking in on a legacy hive ("Bonnie," not owned by the test account) correctly updated status to "Uncertain" and set `last_verified_at` to today. **Worth a mention to Ronnie for the record:** Validate — the app's primary call to action per spec — may have been non-functional for real users prior to this fix; worth considering whether any check-in attempts were lost/frustrated during that window.
...
> **Verification addendum (2026-07-25, Sonnet implementation session):** The checked-in `v2_6_sync.sql` never reflected the fix described above — it still showed `p_hive_id uuid`, `security invoker`, and a grant to `anon, authenticated`. That's a real repo/production drift (the fix was applied live via SQL Editor and only ever described in this doc's prose, never backported to a migration file), not a sign the fix didn't happen. Verified live via three queries Ronnie ran on 2026-07-25: `select * from pg_policies where tablename='hives'` confirmed `hives_auth_update` is `using (submitted_by = auth.uid()) with check (submitted_by = auth.uid())` for `authenticated`; `pg_get_functiondef('public.submit_checkin'::regproc)` confirmed `p_hive_id bigint`, `SECURITY DEFINER`, `search_path` pinned to `public, pg_temp`, and an `auth.uid() is null` guard as the function's first statement; `information_schema.role_routine_grants` showed EXECUTE granted to `service_role`, `authenticated`, `postgres`, **and `PUBLIC`** — meaning `anon` still had EXECUTE via the `PUBLIC` grant (Postgres's default for new functions) even though no explicit `anon` row existed. The `auth.uid()` guard blocks any anon call before it does anything, so this was not an active hole, but it was one edit-away from becoming one with no other signal. **Fixed:** added `v2_9_checkin_security_fix.sql` — a new, idempotent migration that documents the live state (so the repo stops lying about what's in production) and additionally revokes EXECUTE from `PUBLIC`/`anon`, granting only to `authenticated`. 2b is now fully closed at both the RLS and RPC-grant level.
...
> **Implementation-session addendum (2026-07-23, Sonnet session):** Shipped in v2.9.8. Added `showToastWithRetry(msg, retryFn)` alongside the existing `showToast()` — same element, but re-enables `pointer-events` (the base `#toast` rule is `none` so normal toasts don't block map taps), stays up 8s instead of 2.5s, and renders a "Retry" button. The cold-load loop now tracks `loadError` separately from an empty result, so a first-fetch failure (`all.length === 0 && loadError`) shows the retry toast while a genuinely empty table still shows the normal "0 hives loaded" toast. **Scope note:** only wired up for the complete-failure case (page 1 itself errors) — a partial failure (some pages loaded, then an error on a later page) still falls through to the plain success toast showing however many hives *did* load, same as before. That's an existing, unflagged edge case, not a regression; retry there would need marker dedup by `id` to avoid duplicating already-added pins, which felt like scope creep for a low-frequency case. Retry action is a full `location.reload()` rather than re-invoking `loadHivesFromSupabase()` directly, for the same dedup-safety reason. Flagging for the next Fable review if partial-load resilience becomes worth the complexity.
...
- **5a. 🟡 Map doesn't reflow to fill the viewport after a window resize.** ✅ After the browser window changed size mid-session, the Leaflet map stayed at its old dimensions, leaving a large blank grey area to the right and below (see repro: it happened every time the viewport dimensions changed). `map.invalidateSize()` is only called inside `setTab('map'|'validate')` (`app.js:1055`), not on `window.resize`/`orientationchange`. **This app is mobile-first and iPhone rotation is a resize** — a user who rotates their phone, or a desktop user who resizes, can land on a half-rendered map. Recommend a `window.addEventListener('resize'/'orientationchange', debounce(() => map.invalidateSize()))`. Worth confirming on a real iPhone since that's your primary target.
  ...
  > **Implementation-session addendum (2026-07-23, Sonnet session):** Shipped in v2.9.8, in `init()` — a 150ms-debounced handler on both `resize` and `orientationchange` calling `map.invalidateSize()`. No deviation from the recommendation. **Still needs Ronnie's real-iPhone confirmation** per the original note — can't verify rotation behavior from a code review or a desktop browser resize alone.
