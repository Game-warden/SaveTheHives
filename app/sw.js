...
const CACHE_VERSION = 'v2.11.3'; // Bumped for the About panel's conversion from a .modal-overlay bottom sheet to a full #about-view (same architecture as #learn-view) — fixes the persistent header and floating bottom-nav pill being dimmed/blurred behind About's old modal backdrop, and reuses Learn's .lv-card component classes for the hub menu so the "indented style" is consistent across tabs. Touches app/index.html, app.js, and styles.css. Previous bump (v2.11.2) was the About panel reorg (grouped hub + sub-panels) and the ?onboard= nav-pointer callout. App moved off root to /app/ — this file now lives at /app/sw.js so its default scope is /app/*, meaning it never controls the root landing page at all. SHELL_ASSETS below updated to match the new /app/ paths; shared root-level assets (styles.css, manifest.json, icons, logo.jpg) are unchanged since those files did not move.
