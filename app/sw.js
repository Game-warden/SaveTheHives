...
const CACHE_VERSION = 'v2.11.7'; // Re-bump, same-day as v2.11.6 — a genuinely new failure mode caught live: the v2.11.6 install ran during the ~20-30s window right after that commit was pushed, before Cloudflare Pages had finished propagating the new app.js to the origin. The resulting cache was correctly *named* savethehives-shell-v2.11.6, but precached the *stale* app.js bytes fetched during that propagation gap — since the cache name matched what the SW expected, it never re-checked. This is a variant of the addAll()-HTTP-cache bug from Jul 23 2026 (see the install handler's own comment below), except {cache:'reload'} doesn't help here — the fetch genuinely was "fresh," just fresh against an origin that hadn't finished deploying yet. Lesson: right after a push, wait for a cache-busted direct-origin fetch to confirm the new bytes are actually live before treating a device as caught up — a "successful" update install a few seconds too early can silently lock in stale content under the new version name, and only a further bump forces a re-fetch. Previous bump (v2.11.6) was the persistent-header home-link feature itself — touches app.js and styles.css.
