...
const CACHE_VERSION = 'v2.11.8'; // Header home-link hardened for iOS Safari: swapped the plain click-listener-on-a-div (v2.11.6) for a real absolutely-positioned <a> overlay — a JS click listener on a non-interactive element is a known-flaky pattern on iOS Safari even with cursor:pointer set, whereas a genuine anchor's native tap-to-navigate doesn't depend on JS event dispatch at all. Reported not working on an iPhone (both from the home-screen icon, which is correct/expected — see isStandalone() guard — and from Safari itself, which was the real bug this fixes). Touches app.js and styles.css. Previous bump (v2.11.7) re-fetched after a deploy-propagation race left stale bytes in the v2.11.6 cache.
