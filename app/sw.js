...
const CACHE_VERSION = 'v2.11.9'; // Two field-test fixes from real testers (Jul 29 2026): (1) the "check your email" message after Send Link was a 2.5s toast easy to miss entirely — a tester didn't realize he needed to do anything with the email (which had also landed in Spam, undetected separately). Replaced with #email-sent-callout: a dismissible, 12s-minimum banner that explicitly names the Spam folder. (2) The install-instructions fallback (no beforeinstallprompt captured, not iOS) just said "usually Add to Home Screen or Install App from your browser's menu" — a tester on Chrome couldn't find either. Spelled out the actual desktop (address-bar install icon / ⋮ menu) and mobile (⋮ menu) paths explicitly; also split out Chrome-on-iOS from Safari-on-iOS (isIOSChrome()), since it was previously getting Safari-specific "toolbar" wording that doesn't match Chrome's UI. Touches app.js, app/index.html, and styles.css. Previous bump (v2.11.8) hardened the header home-link for iOS Safari.
