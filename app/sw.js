...
const CACHE_VERSION = 'v2.11.6'; // Bumped for the persistent header becoming a clickable shortcut back to the root landing page (browser-tab visitors only — see isStandalone() guard in initHeaderHomeLink(), app.js) — touches app.js and styles.css. Previous bump (v2.11.5) was the logo/icon/honeycomb-photo asset swaps. App moved off root to /app/ — this file now lives at /app/sw.js so its default scope is /app/*, meaning it never controls the root landing page at all. SHELL_ASSETS below updated to match the new /app/ paths; shared root-level assets (styles.css, manifest.json, icons, logo.jpg) are unchanged since those files did not move.
