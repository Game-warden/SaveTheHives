...
const CACHE_VERSION = 'v2.11.5'; // Bumped because two precached SHELL_ASSETS changed bytes without changing filename: /logo.jpg was replaced with a cleaner high-res source (same bee+compass design, icon-192.png/icon-512.png regenerated from the same source too), and /images/honeybee-on-comb.jpg was replaced with a sharper photo (now actually referenced again, in About's Mission & Research panel — see app/index.html). Same-name asset swaps are exactly the case CACHE_VERSION exists for: without this bump, returning visitors' cached shell would keep serving the old logo/photo bytes indefinitely. Previous bump (v2.11.4) capped #about-view's width to match #learn-view's centered column. App moved off root to /app/ — this file now lives at /app/sw.js so its default scope is /app/*, meaning it never controls the root landing page at all. SHELL_ASSETS below updated to match the new /app/ paths; shared root-level assets (styles.css, manifest.json, icons, logo.jpg) are unchanged since those files did not move.
