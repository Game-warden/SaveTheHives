...
| v2.9.4 | Jul 23 2026 | **Open Graph + Twitter Card metadata.** `index.html` had zero social-share metadata before this — no `og:image`, `og:title`, `og:description`, `twitter:card`, and no page `<title>` beyond the bare app name or `<meta name="description">` either. Added a full set using `facebook_cover_photo.jpg` (1702×630 — wider than the ideal 1.91:1 OG ratio; usable but could stand a proper 1200×630 crop later) as the share image. Known limitation: this is one static, site-wide card — the app has no server-side rendering or edge functions, so per-hive share links (if that ever gets built — no `?hiveId=` deep link exists today) can't get their own dynamic preview without adding a rendering layer that doesn't currently exist. |
| v2.9.3 | Jul 22 2026 | **Deep-link tabs + a partial Send Link fix.** Added `?tab=` URL support (`map`/`add`/`validate`/`learn`/`about`) so external links (Facebook posts, etc.) can open directly on a specific tab — previously only `?pf=1` (Pathfinder field-test gate) existed. Also addressed one specific cause of intermittent "Send Link" sign-in failures: the invisible Turnstile challenge had a 6-second client-side timeout, and on slower networks or with ad-blockers/VPNs/DNS filtering interfering with `challenges.cloudflare.com`, that timeout could fire before a token arrived — the code then called `signInWithOtp()` with no captcha token anyway, which Supabase's Attack Protection rejects outright since a token is required. Timeout extended to 15s, and `submitSignIn()` now refuses to send the request at all without a token, showing a clear error instead of a confusing rejection. **This did NOT fully resolve the problem** — see the Known Issues entry below. A recurrence on Jul 23 2026 showed no client-side error at all (request appeared to succeed) but the email still never arrived, which points at a different, downstream cause (deliverability/Resend, Supabase OTP cooldown, or spam filtering) rather than the Turnstile race this version fixed. Under active investigation. |
...
| v2.9.5–v2.10.1 | Jul 23 2026 | **Fable audit implementation (Sonnet session) + hive deep links.** Full findings doc (`FABLE_AUDIT_FINDINGS_2026-07-23.md`) implemented and closed: stored XSS fix (`escapeHtml()`), `hives` UPDATE RLS tightened to owner-scoped + `submit_checkin()` converted to `SECURITY DEFINER` (also caught and fixed a dormant pre-existing bug — `submit_checkin`'s `p_hive_id` was typed `uuid` against a live `bigint` column, meaning every check-in had silently failed since the v2.6 rollout), `emailRedirectTo` query-stripping, supabase-js pinned + multi-tab warning toast, sign-in re-entrancy guard, cold-load error+retry toast, resize/orientationchange → `map.invalidateSize()`, plus low-priority cleanup (Turnstile token nulling, delta-sync `.gte`, `submitHive` try/catch, global `unhandledrejection` handler). Separately, fixed a live bug found during dashboard testing: `navigator.share()` to Messages on macOS arrived as a completely blank message (known WebKit bug combining separate `text`/`url` fields) — fixed by folding the URL into `text`. **Also shipped real hive deep-linking**, closing the v2.9.2/v2.9.4 "no `?hiveId=` deep link exists" gap noted below: hive `id` now shown in the popup and Records list as `#<id>`, `shareHive()` links carry `?hive=<id>`, and a new `flyToHive()` (via the cluster group's `zoomToShowLayer`) opens that exact pin's popup on load. |
...
| v2.11.3–v2.12 | Jul 28-29 2026 | **About panel finished (modal→full-view, width fix), 4 new photos/logo integrated, landing-page teaser swapped for real video, header→landing-page shortcut added, install instructions made browser-specific, sign-in visibility fixed (spam mention + longer callout).** See individual `CACHE_VERSION` comments in `app/sw.js` (v2.11.5–v2.12.0) for per-change detail; not narrated separately here to avoid duplicating the sw.js changelog. |
| v2.13.1 | Jul 30 2026 | **Two RLS/security fixes, applied directly via Supabase MCP** (see `v2_13_1_security_advisor_fixes.sql`). (1) `feature_ideas_with_votes` was a `SECURITY DEFINER` view, flagged CRITICAL by Supabase's own Security Advisor — switched to `security_invoker = true`; verified safe since both underlying tables already grant public `SELECT`. (2) Found while reviewing every RLS policy on every table in response to that report: `hives_auth_insert` had an unconditional `with_check (true)`, meaning any signed-in user could insert a hive with an arbitrary `submitted_by` — a real gap given the new contact-relay feature trusts `submitted_by` to mean "the person who agreed to be contacted." Tightened to `submitted_by = auth.uid()`, matching the already-correct update policy. Confirmed fixed via a follow-up advisor re-check. **Same day, three Cloudflare-side hardening changes** (dashboard, not git-tracked): DNSSEC enabled for `savethehives.org` (Cloudflare is both registrar and DNS provider here, so the DS record was published automatically, no manual registrar step needed); `_dmarc` TXT record tightened from `p=none` (monitor-only) to `p=quarantine` (SPF/DKIM were already correctly configured underneath, so this was a low-risk enforcement change, not a fresh setup); Bot Fight Mode enabled under Security → Settings → Bot traffic. All three were found by manually walking Cloudflare's Security Overview/SSL/DNS settings pages directly (not via Cloudflare's own "Ask AI" dashboard agent, which requires a broad, standing account-wide read token deliberately left disconnected — see chat 2026-07-30 if that tradeoff needs revisiting). |
| v2.13 | Jul 29-30 2026 | **Contact-submitter relay + opt-in, and a first-party visit counter.** Ships the "optional submitter contact info" feature from the Known Issues backlog below (now marked resolved there) — `allow_contact` opt-in checkbox on Add, a "📬 Contact Submitter" popup button, and the `contact-submitter` Edge Function relay (full writeup under "Contact Submitter — Relay Edge Function" above). Separately, added `functions/_middleware.js` — a Cloudflare Pages Function tallying visits by US state/city in Workers KV, with zero client-side script (see "Visit Counter" section above). Both features required real debugging beyond the code itself: Cloudflare Turnstile's hostname allow-list and Supabase's Auth redirect-URL allow-list both had to be extended to cover `*.pages.dev` preview deployments before sign-in worked on preview URLs at all (previously only `savethehives.org`/`www`/`localhost` were listed); the KV binding needed adding to both the Production and Preview environments separately in the Pages project settings, a split that isn't obvious from the UI; and Cloudflare Pages' browser-based function editor had two rough edges worth knowing about if reused — the deploy entrypoint requires the file to stay named exactly `index.ts` regardless of what the function itself is named, and a fresh binding doesn't apply until the deployment is explicitly retried. |
| v2.11 | Jul 28 2026 | **Root becomes a landing page; the app moves to `/app/`.** `/` is now a static, self-contained landing page (`index.html` at repo root, same no-build convention as `privacy.html`) aimed at first-time visitors — mission hook, "three ways to help" cards (Validate first, per Dr. Tarpy's own advice — see `MEETING_NOTES_2026-07-21_Tarpy.md`), and an on-page "why non-managed colonies matter" section. The actual app (map/Validate/Add/Learn) now lives entirely under `/app/` — `index.html`, `app.js`, `pathfinder.js`, `sw.js` all moved there, giving `sw.js` a default scope of `/app/*` so it never touches the landing page. Shared root-level assets (`styles.css`, `manifest.json`, icons, `logo.jpg`) stayed put and are referenced by root-absolute path from both pages. `manifest.json` `start_url`/`scope` now point at `/app/`, so installed users open straight to the map. **Landing page design:** floating glass panel (`.panel`, `max-width:560px`, translucent + blurred) over a full-page fixed meadow-photo background (`images/meadow-hero.jpg`), replacing an earlier solid-black design — considered and rejected a sepia/vintage filter treatment in favor of the original vibrant photo. "Why non-managed colonies matter" redesigned for scannability: bold lede line, two stat callout cards (30–50%, 3+ winters), three colored icon chips (Trees/Old barns/Ground, matching the app's actual pin-legend colors) illustrating where non-managed colonies live, with "non-managed colonies" and "researcher" (was "lab") bolded on first/key use. **Landing ↔ app connective tissue:** each "way to help" card now carries a tag naming the matching bottom-nav tab (Check In tab / Add tab / Learn tab) and links with `?onboard=validate|add|learn`; arriving via that param shows a one-time floating callout bubble pointing at the matching nav button ("Tap Check In to validate an existing hive") instead of the generic first-visit onramp overlay. Mission panel in the app now links back to `/#why-it-matters` so there's a way back to the landing page's story from inside the app. **Orphaned root-scope service worker bug, found and fixed same day:** deleting the old root `sw.js` outright (rather than leaving a stub) broke the self-update path for every browser that had a pre-v2.11 service worker registered at scope `/` — see Known Gotchas below for the full writeup and the fix (a "killer" script at root `sw.js` that unregisters itself and clears old caches). **About panel reorganized twice this session:** first into a grouped hub menu (App / Map reference / Community / About the project sections, each row using Learn's `.lv-card` component — icon square, title, description, chevron — instead of flat list rows), then converted from a `.modal-overlay` bottom sheet into a full `#about-view` (same architecture as `#learn-view`) so the persistent header and floating bottom-nav pill stay crisp instead of dimming behind a modal backdrop, matching Map and Learn. A follow-up fix added `#about-view` to `#learn-view`'s existing `max-width:560px; margin:0 auto` rule, which it hadn't inherited automatically (see Known Gotchas). App version badge bumped v2.10 → v2.11 to match. `CACHE_VERSION` bumped four times this round (`v2.11.1` → `v2.11.4`) as commits touched shell files. |
...
- **Post-push propagation race can lock stale bytes into a correctly-named cache** (discovered 2026-07-28, v2.11.6→v2.11.7) — verifying a deploy within the first ~20-30 seconds after a push can itself cause a new failure mode. If a service-worker install cycle runs *during* that window, `{cache:'reload'}` still fetches "fresh" bytes, but fresh from an origin that hadn't finished propagating yet — so the precached shell ends up genuinely stale even though the cache is named after the correct new `CACHE_VERSION`. Because the name matches what the SW expects, it won't re-check on its own; the "reload twice" trick doesn't rescue it either, since that trick assumes the *origin* already has the new bytes. Symptom: `caches.keys()` shows the expected new version name, but `cache.match()` on a shell asset returns old content. Fix when this happens: bump `CACHE_VERSION` again once the origin is confirmed stable (verify with a cache-busted direct fetch — `fetch(url + '?bust=' + Date.now(), {cache:'no-store'})` — showing the new bytes before trusting any client-side check). **Practical takeaway: don't verify a fresh deploy immediately after confirming the push succeeded — give Cloudflare Pages a beat (concretely confirm via a cache-busted origin fetch) before reloading the live site to check.**
...
- **Orphaned root-scope service worker (discovered 2026-07-28, v2.11.2)** — the v2.11 restructure deleted the root `sw.js` outright when the real service worker moved to `/app/sw.js`. That broke the update mechanism for anyone who had already visited the site before v2.11 shipped: their browser had a service worker registered at scope `/` with active script `https://savethehives.org/sw.js`, and a service worker can only learn a newer version exists by re-fetching its own script URL and diffing the bytes — with nothing left at `/sw.js` (a 404), there was nothing to diff against, so those browsers kept running the old registration forever. No amount of reloading fixes this; it needs an explicit unregister. Confirmed live the same day on both a Chrome testing profile and Ronnie's phone — both stuck showing the pre-v2.11 dark landing page no matter how many reloads, until manually cleared. **Fix shipped:** a tiny "killer" script was put back at root `sw.js` (v2.11.2) — not the real app worker, just cleanup logic (delete all this origin's caches, unregister itself, force one reload) so any browser with the old registration self-heals automatically on next visit. See the comment block at the top of `sw.js` for the full explanation. Leave that file in place permanently as a safety net; nothing re-registers it going forward since the landing page has no service worker of its own by design. **Lesson for any future service-worker scope migration:** deleting the old script outright breaks the self-update path for existing registrations — always leave a short-lived killer script at the old path instead of just removing the file.
...
- **CSS rules scoped to one id don't carry over when a second element adopts the same layout pattern** (discovered 2026-07-28, v2.11.4) — when the About panel was converted from a modal into a full view matching `#learn-view` (see the v2.11 changelog entry above), the new `#about-view` element used the same structural CSS class (`.learn-screen`) as Learn, but the *width cap* (`max-width:560px; margin:0 auto`) was written as `#learn-view { ... }`, a rule scoped to that one id. `#about-view` didn't inherit it just by looking similar — it stretched edge-to-edge on desktop while Learn stayed in a narrow centered column, a visible inconsistency Ronnie caught immediately after the panel otherwise looked right. Fixed by changing the selector to `#learn-view, #about-view { ... }`. Takeaway for any future "make X look like Y" conversion: id-scoped rules need to be explicitly extended to the new element (or the shared styling moved to a class both elements carry) — visually copying another element's child classes isn't enough if the parent's own sizing rule is id-scoped.
...
- **UNRESOLVED — recurring "Send Link" sign-in failure (Jul 22–23 2026).** Not fixed by v2.9.3, which only addressed one specific cause. Timeline: (1) Ronnie reported intermittent failures, first reproduced at home Jul 21; (2) code review found the invisible Turnstile challenge had a 6s client-side timeout, and if it lost that race, `signInWithOtp()` was called with no captcha token anyway and got rejected by Supabase's Attack Protection — this was fixed in v2.9.3 (timeout to 15s, request no longer sent without a token); (3) Ronnie confirmed Send Link "seems to be working now" after that fix; (4) **it recurred Jul 23, same home network** — clicked Send Link, no error shown (implying the client-side request succeeded this time), but the email never arrived. That's a different failure mode than what v2.9.3 fixed — something between Supabase accepting the request and the email reaching the inbox. Leading theories, unconfirmed: (a) landed in spam/junk — quickest thing to check; (b) Resend actually failed/bounced the send — only visible in Resend's own dashboard (resend.com → Emails/Logs), which nobody has checked yet; (c) Supabase's built-in OTP resend cooldown (period each) silently no-ops a repeat request to the same email without erroring, so quick repeated attempts might look like "it did nothing" — needs testing with a single clean attempt and Supabase Authentication → Logs checked afterward; (d) Resend/domain deliverability reputation — `savethehives.org` is a relatively young sending domain, worth checking SPF/DKIM/DMARC are all still correctly verified in Resend, not just present. **Nobody has checked the Resend or Supabase dashboards yet — that's the highest-value next step, and needs Ronnie's login.** Given Ronnie has ~188 pending friend invites riding on trusting this flow, this is the top-priority item for any audit/debugging session.
- **Validated-hive visual credit + filter (discussed Jul 22 2026)** — not built, no code yet, design still open. Motivation: with Validate as the primary call to action, hives that have actually been checked on deserve visible distinction from the ~1,150+ legacy records nobody's confirmed since 2008–2017.
  - **Marker treatment: checkmark badge, decided.** A small badge (e.g. green circle + check) added to the existing pin icon in `makeIcon()`/`addMarker()`. Considered a static halo ring and a live CSS/SVG pulse too (mocked up in chat) — pulse was rejected as unnecessary complexity and a performance risk at scale (animating hundreds of DOM markers simultaneously compounds the same low-end-device concern already flagged for the mating-radius overlay). Badge = "this hive has been checked on." The existing status color-coding in the popup (green/red/amber for active/gone/uncertain) already conveys the outcome separately — the badge shouldn't try to encode both.
  - **What counts as "validated": any check-in, not just a positive one.** `status !== 'unverified'` (i.e. active, gone, OR uncertain) all count — the check-in event itself is the validation, not the outcome. A confirmed *loss* is genuinely useful research data (it's the survival/attrition signal the whole Genetic Goldmine thesis depends on), so "gone" shouldn't be treated as a lesser or non-validation.
  - **The filter needs a decision: it may actually be two different filters.** (1) Filter *to* already-validated hives — a research tool for finding candidates worth deeper attention (a hive confirmed active across multiple check-ins over separate years is exactly what a "3+ winter survivor" looks like). (2) Filter *to* unvalidated/stale hives — a volunteer-facing tool, closer to what the Validate tab's existing stale-record banner already gestures at, just not exposed as a standalone filter. These serve different audiences (researcher vs. volunteer) and haven't been narrowed down to one yet.
  - **Open proximity/trust question, raised but not resolved.** As validation becomes something with visible payoff (a badge, a research filter scientists rely on), a self-report check-in with no location verification becomes a softer spot than it was when Validate was just "keep records current." This directly reopens the shelved Jul 17 2026 anti-fraud/proximity design just above — that plan already worked out the hard parts (live GPS at check-in time via the existing `haversine()` helper, not photo EXIF, which is unreliable and stripped by compression; a generous 100+ yard threshold given the field-tested ±20m GPS accuracy floor under canopy). Still undecided: does the badge/filter require a proximity-confirmed check-in specifically, or does any check-in qualify regardless of whether proximity was ever checked?
- **Optional submitter contact info — SHIPPED 2026-07-29/30 (v2.13).** Built as proposed: `allow_contact` boolean on `hives`, opt-in checkbox on the Add form, and a "📬 Contact Submitter" popup button that only appears on opted-in hives (hidden on your own). Went with the relay approach from the original open question — the `contact-submitter` Edge Function looks up the submitter's email from `auth.users` server-side and never exposes it to the client; the sender's email only reaches the submitter as a reply-to on the relayed email. Applies only to new submissions through the current form (legacy records were never retrofitted — not raised as a need). `privacy.html` was updated with a new disclosure section. Full architecture documented under "Contact Submitter — Relay Edge Function" above.
