...
- **Optional submitter contact info (proposed Jul 22 2026)** — not built. Motivation: with Validate now the project's primary call to action (see `CONTENT_LIBRARY_IDEAS.md` #15 and `FACEBOOK_STARTER_PACK.md` §4), someone heading out to confirm a hive that's on private property currently has no way to reach whoever originally logged it to ask permission. Proposal: add optional `contact_email` / `contact_phone` fields to the Add Hive form, gated behind an explicit opt-in checkbox ("I'm willing to be contacted about this hive") — off by default, not required. Open design questions for whoever builds this: (1) whether to show the raw contact info on opted-in hives, or route through a "Contact submitter" mailto/relay to cut down on scraping — `hello@savethehives.org` already shows the relay pattern the project uses elsewhere; (2) whether this applies only to new (`user_added = true`) submissions or could also be retrofitted onto legacy records if a submitter re-confirms and opts in during a future check-in; (3) privacy policy (`privacy.html`) will need a new section once this ships, since it currently only covers the auth email (admin-only) and location data, not user-visible contact info. Note: a handful of the 1,162 legacy 2008-era notes already contain informal contact info typed directly into the free-text notes field (phone numbers, "email me" invitations) — those predate any opt-in mechanism and are a useful real-world reference for what the feature should formalize.
